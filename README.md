# sshgen — SSH Key Lifecycle Management

Store, organize, and export SSH keys in a local SQLite database.

```
sshgen import mykey -i ~/.ssh/
sshgen list
sshgen export mykey -o ~/.ssh/
sshgen rename old new
sshgen delete mykey
```
...
```bash
sshgen import mykey -i ~/.ssh/
sshgen import mykey --priv ~/.ssh/id_ed25519 --pub ~/.ssh/id_ed25519.pub
```
...
```bash
sshgen export mykey -o ~/.ssh/
sshgen export mykey --priv ~/.ssh/mykey --pub ~/.ssh/mykey.pub
```
...
```bash
git clone https://github.com/yourusername/sshgen.git && cd sshgen
go build -o sshgen .
```
